...
public static class Plot
{
    ...
    public static Image PlotGraphs(IEnumerable<IEnumerable<PointF>> points, int width, int height, Color? color = null)
    {
        ...
        g.Transform = new Matrix(1, 0, 0, -1, 0, bmp.Height); ;
    ...
    private static void PlotAxes(Graphics g, BoundsF bounds, double scale, Bitmap bmp)
    {
    ...
    private static void LabelXRule(double v, Graphics g, BoundsF bounds, double scale)
    {
        ...
        g.FillRectangle(Brushes.White, x, -txtSize.Height, txtSize.Width, txtSize.Height);
        g.DrawString(label, font, Brushes.Gray, x, -txtSize.Height);
        g.Restore(gs);
    }
    ...
    private static void LabelYRule(double v, Graphics g, BoundsF bounds, double scale)
    {
        ...
        if (y > txtSize.Height * 1.5)
        {
            var gs = g.Save();
            g.ScaleTransform(1.0f, -1.0f, MatrixOrder.Prepend);
            g.FillRectangle(Brushes.White, 0, -y - txtSize.Height, txtSize.Width, txtSize.Height);
            g.DrawString(label, font, Brushes.Gray, 0, -y - txtSize.Height);
            g.Restore(gs);
        }
    }
